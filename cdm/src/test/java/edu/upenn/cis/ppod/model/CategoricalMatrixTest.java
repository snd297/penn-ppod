...
public class CategoricalMatrixTest {
	...
	@Inject
	private Provider<CategoricalMatrix> categoricalMatrixProvider;

	@Inject
	private Provider<Character> categoricalCharacterProvider;
	...
	public void beforeMethod() {
		matrix = categoricalMatrixProvider.get();
		...
		final ImmutableSet<CategoricalMatrix> matrices = ImmutableSet
				.of(matrix);

		otuSet012.setCategoricalMatrices(matrices);
	...
	public void setOTUsWReorderedOTUs() {
		...
		final Character character = categoricalCharacterProvider
				.get();
		character.setLabel("testLabel");
		matrix.setCharacters(newArrayList(character));

		matrix.putRow(otu0, rowProvider.get());
		final CategoricalCell cell00 = cellProvider.get();
		matrix.getRow(otu0).setCells(
				Arrays.asList(new CategoricalCell[] { cell00 }));
		cell00.setSingleState(stateFactory.create(0).setCharacter(character));

		matrix.putRow(otu1, rowProvider.get());
		final CategoricalCell cell10 = cellProvider.get();
		matrix.getRow(otu1).setCells(
				Arrays.asList(new CategoricalCell[] { cell10 }));

		final CategoricalState state1 = stateFactory.create(1);
		character.addState(state1);
		cell10.setSingleState(state1);

		matrix.putRow(otu2, rowProvider.get());
		final CategoricalCell cell20 = cellProvider.get();
		matrix.getRow(otu2).setCells(
				Arrays.asList(new CategoricalCell[] { cell20 }));

		final CategoricalState state0 = stateFactory.create(0);
		character.addState(state0);
		cell20.setSingleState(state0);

		final List<CategoricalRow> originalRows = newArrayList(matrix);
	...
	public void setCharacters() {
		...
		final ImmutableList<Character> characters = ImmutableList
				.of(
						(Character) categoricalCharacterProvider
								.get().setLabel(
										"character0"),
						(Character) categoricalCharacterProvider
								.get().setLabel(
	...
	public void moveCharacter() {
		...
		final ImmutableList<Character> shuffledCharacters = ImmutableList
				.of(
	...
	public void replaceCharacters() {
		...
		final ImmutableList<Character> characters2 = ImmutableList
				.of(
						(Character) categoricalCharacterProvider
								.get().setLabel(
										"character2-0"),
	...
	public void setWithSameRow() {
		final CategoricalRow row1 = rowProvider.get();
...
// }
	...
	public void replaceRow() {
		final CategoricalRow row1 = rowProvider.get();
		matrix.putRow(otu1, row1);
		final CategoricalRow row2 = rowProvider.get();
		final CategoricalRow someRow = matrix.putRow(otu1, row2);
	...
	public void setCharactersW2EqualsCharacters() {
		final Character character0 = categoricalCharacterProvider
				.get();
		character0.setLabel("character0");
		final Character character1 = categoricalCharacterProvider
				.get();
	...
	public void setColumnPPodVersionInfos() {
		...
		final Matrix returnedMatrix = matrix
