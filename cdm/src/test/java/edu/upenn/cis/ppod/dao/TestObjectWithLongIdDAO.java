...
public class TestObjectWithLongIdDAO implements IObjectWithLongIdDAOHibernate {
	...
	private Map<CategoricalRow, List<CategoricalCell>> rowsToCells = newHashMap();
	...
	public void saveOrUpdate(final Object entity) {
		if (entity instanceof CategoricalCell) {
			final CategoricalCell cell = (CategoricalCell) entity;
			final CategoricalRow row = cell.getRow();
			if (rowsToCells.containsKey(cell.getRow())) {

			} else {
				rowsToCells.put(row, new ArrayList<CategoricalCell>());
			}
	...
	public Map<CategoricalRow, List<CategoricalCell>> getRowsToCells() {
		return rowsToCells;
	}
