...
public final class CharacterStateRow extends PPodEntity {
	...
	/**
	 * Position of cells in this {@code CharacterStateRow}. Intentionally
	 * package-private.
	 */
	static final String CELL_INDEX_COLUMN = CharacterStateCell.TABLE
			+ "_POSITION";

	/** The {@code CharacterStateCell}s that make up the row. */
	@XmlElement(name = "cell")
	@OneToMany
	@org.hibernate.annotations.IndexColumn(name = CELL_INDEX_COLUMN)
	...
	public CharacterStateCell addCell(final CharacterStateCell cell) {
		...
		if (getMatrix().getCharacters().size() < getCells().size() + 1
				|| (getMatrix().getCharacters().size() > 0 && getMatrix()
						.getCharacters().get(getCells().size()) == null)) {
			throw new IllegalStateException(
